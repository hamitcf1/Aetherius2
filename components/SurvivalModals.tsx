...
export function RestModal({ open, onClose, onRest, gold, hasCampingGear, hasBedroll }: RestModalProps) {
  ...
  const restQuality = useMemo(() => {
    if (restType === 'inn') return { label: t('rest.quality.well'), fatigueReduction: 50, desc: t('rest.quality.wellDesc') };
    if (restType === 'camp') {
      if (hasCampingGear) return { label: t('rest.quality.normal'), fatigueReduction: 40, desc: t('rest.quality.normalDesc') };
      if (hasBedroll) return { label: t('rest.quality.somewhat'), fatigueReduction: 30, desc: t('rest.quality.somewhatDesc') };
    }
    return { label: t('rest.quality.poor'), fatigueReduction: 15, desc: t('rest.quality.poorDesc') };
  }, [restType, hasCampingGear, hasBedroll, t]);
      ...
      <div className="w-full max-w-md bg-skyrim-paper border border-skyrim-gold rounded-lg shadow-2xl overflow-hidden">
        <div className="p-4 border-b border-skyrim-border flex items-center justify-between bg-skyrim-dark/50">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-serif text-skyrim-gold">{t('rest.title')}</h2>
          </div>
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="space-y-2">
            <label className="text-xs uppercase tracking-wider text-skyrim-text font-bold">{t('rest.location')}</label>
            <div className="grid grid-cols-3 gap-2">
              <button
                onClick={() => setRestType('outside')}
                className={`p-3 rounded border flex flex-col items-center gap-2 transition-colors ${restType === 'outside'
              ...
              >
                <TreePine size={20} />
                <span className="text-xs">{t('rest.locations.outside')}</span>
              </button>
              <button
                onClick={() => setRestType('camp')}
                className={`p-3 rounded border flex flex-col items-center gap-2 transition-colors ${restType === 'camp'
                    ? 'bg-skyrim-gold/20 border-skyrim-gold text-skyrim-gold'
                    : 'bg-skyrim-paper/30 border-skyrim-border text-skyrim-text hover:border-skyrim-border'
                  }`}
              >
                <Tent size={20} />
                <span className="text-xs">{t('rest.locations.camp')}</span>
                {!hasBedroll && !hasCampingGear && (
                  <span className="text-[10px] text-red-400">{t('rest.warnings.noGear')}</span>
                )}
              </button>
              <button
                onClick={() => setRestType('inn')}
                disabled={!canAffordInn}
                className={`p-3 rounded border flex flex-col items-center gap-2 transition-colors ${restType === 'inn'
                    ? 'bg-skyrim-gold/20 border-skyrim-gold text-skyrim-gold'
                    : canAffordInn
                      ? 'bg-skyrim-paper/30 border-skyrim-border text-skyrim-text hover:border-skyrim-border'
              ...
              >
                <Home size={20} />
                <span className="text-xs">{t('rest.locations.inn')}</span>
          ...
          <div className="p-3 bg-skyrim-paper/30 border border-skyrim-border rounded">
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-xs text-green-400">-{restQuality.fatigueReduction} {t('survival.fatigue')}</span>
            </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <label className="text-xs uppercase tracking-wider text-skyrim-text font-bold">{t('rest.hours')}</label>
              <span className="text-sm text-skyrim-text flex items-center gap-1">
                <Clock size={14} /> {hours} {hours !== 1 ? t('rest.labels.hours') : t('rest.labels.hour')}
              </span>
            </div>
            <input
              type="range"
          ...
          >
            {t('rest.actions.confirm')} ({hours} {hours !== 1 ? t('rest.labels.hours') : t('rest.labels.hour')})
          </button>
        </div>
      </div>
    </div>
      ...
      <div className="w-full max-w-md bg-skyrim-paper border border-skyrim-gold rounded-lg shadow-2xl overflow-hidden">
        <div className="p-4 border-b border-skyrim-border flex items-center justify-between bg-skyrim-dark/50">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-serif text-skyrim-gold">{t('survival.eat')}</h2>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-skyrim-paper/40 rounded">
            <X size={18} className="text-skyrim-text hover:text-white" />
          </button>
        ...
        <div className="flex border-b border-skyrim-border bg-skyrim-paper/20">
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-bold transition-colors ${activeCategory === 'food'
          ...
          >
            <Apple size={16} />
            {t('category.food')} ({foodOnly.length})
          </button>
          <button
            onClick={() => setActiveCategory('ingredient')}
            className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-bold transition-colors ${activeCategory === 'ingredient'
                ? 'bg-purple-900/30 text-purple-400 border-b-2 border-purple-500'
                : 'text-skyrim-text hover:text-skyrim-gold hover:bg-skyrim-paper/20'
              }`}
          >
            <Droplets size={16} />
            {t('category.ingredients')} ({ingredientsOnly.length})
          </button>
        </div>
        ...
        <div className="p-4">
          {displayItems.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-skyrim-text">{t('inventory.emptyCategory', { category: activeCategory === 'food' ? t('category.food') : t('category.ingredients') })}</p>
              <p className="text-xs text-skyrim-text mt-1">{t('rest.visitShop')}</p>
            </div>
          ) : (
      ...
      <div className="w-full max-w-md bg-skyrim-paper border border-skyrim-gold rounded-lg shadow-2xl overflow-hidden">
        <div className="p-4 border-b border-skyrim-border flex items-center justify-between bg-skyrim-dark/50">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-serif text-skyrim-gold">{t('survival.drink')}</h2>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-skyrim-paper/40 rounded">
            <X size={18} className="text-skyrim-text hover:text-white" />
          </button>
        ...
        <div className="p-4">
          {availableDrinks.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-skyrim-text">{t('inventory.emptyCategory', { category: t('category.drink') })}</p>
              <p className="text-xs text-gray-500 mt-1">{t('rest.visitShop')}</p>
            </div>
          ) : (
            <div className="space-y-2 max-h-80 overflow-y-auto">
