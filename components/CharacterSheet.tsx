...
const TextAreaField: React.FC<{
  ...
  <div className="flex flex-col gap-2">
    ...
    <textarea
      ...
      className="w-full glass-panel-lighter border-zinc-700/50 rounded p-3 text-skyrim-text focus:border-amber-500 focus:ring-1 focus:ring-amber-500/30 outline-none transition-all placeholder-gray-600 font-sans"
      style={{ color: 'var(--skyrim-text)' }}
      autoCapitalize="none"
      autoCorrect="off"
      rows={rows}
...
export const CharacterSheet: React.FC<CharacterSheetProps> = ({
  ...
  const renderPerks = () => {
      ...
      return (
        <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2">
          {sortedSkills.map(skill => (
            <div key={skill} className="relative pl-4 border-l-2 border-zinc-700/50">
              ...
              <div className="space-y-3">
                {grouped[skill].sort((a, b) => a.rank - b.rank).map(perk => (
                  <div key={perk.id} className="relative glass-panel-lighter p-3 rounded border border-zinc-700/50 ml-2 group hover:border-amber-500/50 transition-colors">
                    <div className="absolute top-1/2 -left-4 w-4 h-px bg-zinc-700/50"></div>
          ...
          {perks.length === 0 && <div className="text-xs text-gray-600 italic">{t('character.noPerks')}</div>}
        </div>
      );
    }
    ...
    return (
      <div className="space-y-2 max-h-[500px] overflow-y-auto pr-2">
        {sorted.map(perk => (
          <div key={perk.id} className="flex justify-between items-center glass-panel-lighter p-3 rounded border border-zinc-700/50 group hover:border-amber-500/50 hover:bg-zinc-800/40 transition-colors">
            <div>
  ...
  return (
    <div className="max-w-4xl mx-auto pb-24">
      ...
      <div id="character-sheet-content" className="p-4 bg-transparent">
        <div className="mb-8 p-6 glass-panel border-zinc-700/50 rounded-xl text-center relative overflow-hidden shadow-2xl">
        ...
        {/* Level and Experience */}            <div className="mb-6 glass-panel border-zinc-700/50 p-4 rounded-xl shadow-lg relative z-10">
          <div className="flex flex-col gap-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              {/* Level Badge */}
              <div className="flex items-center gap-6 relative z-10">
                <LevelBadge level={character.level} size={80} />
              ...
              <div className="flex items-center gap-2 self-start sm:self-center">
                ...
                {levelUpAvailable && (
                  <button onClick={() => onRequestLevelUp && onRequestLevelUp()} className="ml-2 px-3 py-1 rounded bg-skyrim-gold text-skyrim-dark text-xs font-semibold hover:bg-yellow-400 transition-all">{t('character.levelUpAvailable')}</button>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="mt-3 pt-3 border-t border-skyrim-border/30">
            <div className="flex flex-wrap items-center gap-4 text-xs text-gray-500">
              <span>
                <span className="text-skyrim-text">{t('character.levelReq', { level: character.level + 1, xp: getXPForNextLevel(character.level).toLocaleString() })}</span>
              </span>
              <span>•</span>
              <span>
          ...
          <div className="mb-6">
            ...
            >
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="text-gray-300 font-medium">{t('character.maxStats')}</span>
        ...
        <div className="mb-6 p-4 bg-skyrim-paper/40 rounded border border-skyrim-border">
          <div className="text-xs uppercase tracking-widest text-skyrim-text font-bold mb-3">{t('character.combatStats')}</div>
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="flex-1 flex items-center gap-3 p-3 bg-blue-900/20 border border-blue-700/30 rounded">
              ...
              <div>
                <div className="text-xs text-skyrim-text uppercase">{t('character.armorRating')}</div>
            ...
            <div className="flex-1 flex items-center gap-3 p-3 bg-red-900/20 border border-red-700/30 rounded">
              ...
              <div>
                <div className="text-xs text-skyrim-text uppercase">{t('character.weaponDamage')}</div>
          ...
          {equipmentStats.equippedItems.length > 0 && (
            <div className="mt-3 pt-3 border-t border-skyrim-border/30">
              <div className="text-xs text-gray-500 mb-2">{t('character.equipped')}:</div>
        ...
        <div className="mb-6 p-4 bg-skyrim-paper/40 rounded border border-skyrim-border">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <div className="text-xs uppercase tracking-widest text-skyrim-text font-bold">{t('character.inGameTime')}</div>
        ...
        <Section title={t('character.sections.identity')} icon={<User />} defaultOpen={true}>
          <div className="mb-4">
            <label className="text-sm uppercase tracking-wider text-skyrim-text font-bold mb-2 block">{t('character.gender')}</label>
            <DropdownSelector
          ...
          <TextAreaField
            label={t('character.coreIdentity')}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <TextAreaField label={t('character.psychology')} value={character.psychology} onChange={(v) => updateCharacter('psychology', v)} placeholder="Mental state, personality quirks..." />
            <TextAreaField label={t('character.moralCode')} value={character.moralCode} onChange={(v) => updateCharacter('moralCode', v)} placeholder="Lines they will not cross..." />
          </div>
          <TextAreaField label={t('character.breakingPoint')} value={character.breakingPoint} onChange={(v) => updateCharacter('breakingPoint', v)} placeholder="What finally makes them snap?" rows={2} />
        </Section>
        {spellsOpen && (
        ...
        <Section title={t('character.sections.talents')} icon={<Sparkles />}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <TextAreaField label={t('character.talents')} value={character.talents} onChange={(v) => updateCharacter('talents', v)} placeholder="Natural aptitudes (e.g., Keen eye, Steady hand)..." />
            <TextAreaField label={t('character.magicApproach')} value={character.magicApproach} onChange={(v) => updateCharacter('magicApproach', v)} placeholder="Shunned? Embraced? Only Restoration?" />
          </div>
        </Section>

        <Section title={t('character.sections.fears')} icon={<Ghost />}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <TextAreaField label={t('character.fears')} value={character.fears} onChange={(v) => updateCharacter('fears', v)} placeholder="Spiders? Darkness? Failure?" />
            <TextAreaField label={t('character.weaknesses')} value={character.weaknesses} onChange={(v) => updateCharacter('weaknesses', v)} placeholder="Physical or mental flaws..." />
          </div>
        </Section>

        <Section title={t('character.sections.evolution')} icon={<Activity />}>
          <div className="space-y-6">
            <div className="bg-skyrim-paper/30 p-4 border border-skyrim-border/50 rounded mb-4">
              <h3 className="flex items-center gap-2 text-skyrim-gold font-bold uppercase text-sm mb-2">
                <ScrollText size={16} /> {t('character.roleplayBehavior')}
              </h3>
              <TextAreaField label={t('character.forcedBehavior')} value={character.forcedBehavior} onChange={(v) => updateCharacter('forcedBehavior', v)} placeholder="Compulsive behaviors or rituals the user must enact..." />
            </div>

            <TextAreaField label={t('character.longTermEvolution')} value={character.longTermEvolution} onChange={(v) => updateCharacter('longTermEvolution', v)} placeholder="How should the character change over levels 1-50?" rows={4} />

            <div className="bg-skyrim-paper/30 p-4 rounded border border-skyrim-border">
              <h3 className="text-skyrim-gold font-bold uppercase text-sm mb-3">{t('character.evolutionMilestones')}</h3>
              ...
              <div className="space-y-2">
                ...
                {(character.milestones || []).length === 0 && <p className="text-xs text-gray-600 italic">{t('character.noMilestones')}</p>}
              </div>
            </div>
          </div>
        </Section>

        <Section title={t('character.sections.skills')} icon={<Zap />}>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {/* Skills List */}
            <div className="space-y-3">
              <h3 className="text-sm uppercase tracking-widest text-skyrim-gold border-b border-skyrim-border pb-2 mb-4">{t('character.skillProficiency')}</h3>
            ...
            <div>
              <div className="flex justify-between items-center mb-4 border-b border-skyrim-border pb-2">
                <h3 className="text-sm uppercase tracking-widest text-skyrim-gold">{t('character.activePerks')}</h3>
              ...
              <div className="bg-skyrim-paper/20 p-4 rounded border border-skyrim-border/50 mb-4">
                <div className="grid grid-cols-12 gap-2 mb-2">
                  <div className="col-span-8">
                    <label className="text-[10px] uppercase text-gray-500 font-bold">{t('character.perkName')}</label>
                    <input
                ...
                <div className="grid grid-cols-12 gap-2 mb-2">
                  <div className="col-span-3">
                    <label className="text-[10px] uppercase text-gray-500 font-bold">{t('character.perkRank')}</label>
                    <input
                      type="number"
                      min="1"
                      max="5"
                  ...
                  <div className="col-span-9">
                    <label className="text-[10px] uppercase text-gray-500 font-bold">{t('character.perkDesc')}</label>
                    <input
                      className="w-full border border-skyrim-border p-2 rounded text-sm text-skyrim-text focus:border-skyrim-gold focus:outline-none"
                      style={{ backgroundColor: 'var(--skyrim-paper)', color: 'var(--skyrim-text)' }}
                ...
                <button onClick={addPerk} className="w-full py-2 bg-skyrim-gold text-skyrim-dark font-bold rounded hover:bg-skyrim-goldHover flex justify-center items-center gap-2 text-sm">
                  <Plus size={16} /> {t('character.addPerk')}
                </button>
              </div>
        ...
        <Section title={t('character.sections.rules')} icon={<ShieldBan />}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <TextAreaField label={t('character.allowedActions')} value={character.allowedActions} onChange={(v) => updateCharacter('allowedActions', v)} placeholder="Roleplay actions that are encouraged..." rows={5} />
            <TextAreaField label={t('character.forbiddenActions')} value={character.forbiddenActions} onChange={(v) => updateCharacter('forbiddenActions', v)} placeholder="Fast travel? Looting dead bodies? Using healing potions?" rows={5} />
          </div>
        </Section>

        <Section title={t('character.sections.faction')} icon={<Map />}>
          <TextAreaField
            label={t('character.factionAllegiance')}
          ...
          />
          <TextAreaField label={t('character.generalWorldview')} value={character.worldview} onChange={(v) => updateCharacter('worldview', v)} placeholder="Cynical, hopeful, nihilistic..." rows={3} />
          <TextAreaField label={t('character.daedricPerception')} value={character.daedricPerception} onChange={(v) => updateCharacter('daedricPerception', v)} placeholder="How do they view the Princes? Worship one? Hate all?" rows={3} />
        </Section>

        <Section title={t('character.sections.backstory')} icon={<Brain />}>
          <TextAreaField
            label={t('character.fullHistory')}
